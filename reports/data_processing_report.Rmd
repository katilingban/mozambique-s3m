...
The first function recodes the immunisation card retention indicator. The next two functions recode respones to each childhood immunisation dose in the Mozambique national EPI schedule. The fourth function recodes the indicators on full immunisation for children between 1 - 2 years old (by recall and by card), and age-appropriate immunisation for all children in the sample based on official WHO guidance[^9]. The final function provides overall recoding and concatenates the various childhood immunisation indicators with the core variables.

To get the final recoded childhood immunisation dataset, the following call is performed:

```{r epi_function, echo = TRUE, eval = FALSE}
## Immunisation
...
which produces a data.frame with the core variables and the following recoded childhood immunisation indicator values:

```{r epi_indicators}
indicators <- c(
  paste0("imm_card", 1:2), 
  paste0("bcg_card_", c("recall", "only")),
  paste0("opv1_card_", c("recall", "only")),
  paste0("opv2_card_", c("recall", "only")),
  paste0("opv3_card_", c("recall", "only")),
  paste0("opv4_card_", c("recall", "only")),
  paste0("dpt1_card_", c("recall", "only")),
  paste0("dpt2_card_", c("recall", "only")),
  paste0("dpt3_card_", c("recall", "only")),
  paste0("rub1_card_", c("recall", "only")),
  paste0("rub2_card_", c("recall", "only")),
  paste0("pcv1_card_", c("recall", "only")),
  paste0("pcv2_card_", c("recall", "only")),
  paste0("pcv3_card_", c("recall", "only")),
  paste0("vorh1_card_", c("recall", "only")),
  paste0("vorh2_card_", c("recall", "only")),
  paste0("imm_full_", c("recall", "card")),
  paste0("imm_appropriate_", c("recall", "card"))
)

desc <- c(
  "Ever had an EPI card",
  "EPI card retention",
  "Age-appropriate BCG immunisation by card and recall",
  "Age-appropriate BCG immunisation by card",
  "Age-appropriate OPV dose 1 immunisaiton by card and recall",
  "Age-appropriate OPV dose 1 immunisaiton by card",
  "Age-appropriate OPV dose 2 immunisaiton by card and recall",
  "Age-appropriate OPV dose 2 immunisaiton by card",
  "Age-appropriate OPV dose 3 immunisaiton by card and recall",
  "Age-appropriate OPV dose 3 immunisaiton by card",
  "Age-appropriate OPV dose 4 immunisaiton by card and recall",
  "Age-appropriate OPV dose 4 immunisaiton by card",
  "Age-appropriate DPT dose 1 immunisaiton by card and recall",
  "Age-appropriate DPT dose 1 immunisaiton by card",
  "Age-appropriate DPT dose 2 immunisaiton by card and recall",
  "Age-appropriate DPT dose 2 immunisaiton by card",
  "Age-appropriate DPT dose 3 immunisaiton by card and recall",
  "Age-appropriate DPT dose 3 immunisaiton by card",
  "Age-appropriate measles dose 1 immunisaiton by card and recall",
  "Age-appropriate measles dose 1 immunisaiton by card",
  "Age-appropriate measles dose 2 immunisaiton by card and recall",
  "Age-appropriate measles dose 2 immunisaiton by card",
  "Age-appropriate PCV dose 1 immunisaiton by card and recall",
  "Age-appropriate PCV dose 1 immunisaiton by card",
  "Age-appropriate PCV dose 2 immunisaiton by card and recall",
  "Age-appropriate PCV dose 2 immunisaiton by card",
  "Age-appropriate PCV dose 3 immunisaiton by card and recall",
  "Age-appropriate PCV dose 3 immunisaiton by card",
  "Age-appropriate VORH dose 1 immunisaiton by card and recall",
  "Age-appropriate VORH dose 1 immunisaiton by card",
  "Age-appropriate VORH dose 2 immunisaiton by card and recall",
  "Age-appropriate VORH dose 2 immunisaiton by card",
  "Fully immunised children by 1 year by card and recall",
  "Fully immunised children by 1 year by card",
  "Age-appropriate immunisation by card and recall",
  "Age-appropriate immunisation by card"
)
...
The first two functions recode responses to the different breastfeeding indicators questions. The third function maps the variables to each of the breastfeeding indicators. The fourth function recodes exclusive breastfeeding. The fifth function recodes continuing breastfeeding. The sixth function recodes early initiation of breastfeeding. The seventh function recodes the breastfeeding component of the ICFI. The final function provides overall recoding and concatenates the breastfeeding indicators with the core variables.

To get the final recoded child breastfeeding dataset, the following call is performed:

```{r bf_function, echo = TRUE, eval = FALSE}
## Breastfeeding
bf_vars_map <- bf_map_vars(survey_codebook),
bf_recoded_data <- bf_recode(vars = bf_vars_map, .data = raw_data_clean)
```

which produces a data.frame with the core variables and the following recoded child breastfeeding indicator values:

```{r bf_indicators}
indicators <- paste0(
  "bf_", c("ever", "early", "exclusive", "continuing", "icfi")
)
...
The first two functions recode responses for food item questions. The third function maps food items questions to specific child food group variables. The next two functions recode responses to food items questions for the different child food groups. The sixth function calculates the food group score while the seventh function recodes the food group score to the food groups component of the ICFI. The final function provides overall recoding and concatenates the dietary diversity indicators with the core variables.

To get the final recoded child dietary diversity dataset, the following call is performed:

```{r fg_function, echo = TRUE, eval = FALSE}
## Food groups
...
which produces a data.frame with the core variables and the following recoded child dietary diversity indicator values:

```{r fg_indicators}
indicators <- paste0(
  "fg_", c("dairy", "starch", "vita", "other_fruit_veg", "legumes", "meat", "eggs", "score", "icfi")
)

desc <- c(
  "Food group - dairy",
  "Food group - starchy staples",
  "Food group - vitamin A rich fruits and vegetables",
  "Food group - other fruits and vegetables",
  "Food group - legumes, nuts, and seeds",
  "Food group - meat and fish",
  "Food group - eggs",
  "Food group score",
  "Food group - ICFI"
)
...
The first two functions recode responses to the child meal frequency indicator question. The next function recodes the child meal frequency component of the ICFI. The final function provides overall recoding and concatenates the child meal frequency indicators with the core variables.

To get the final recoded child meal frequency dataset, the following call is performed:

```{r meal_function, echo = TRUE, eval = FALSE}
## Meals
meal_recoded_data <- meal_recode(vars = "food_num", .data = raw_data_clean)
```

which produces a data.frame with the core variables and the following recoded child meal frequency indicator values:

```{r meal_indicators}
indicators <- c("meal_frequency", "meal_icfi")

desc <- c(
  "Meal frequency",
  "Meal frequency for ICFI"
)
...
[^9]: World Health Organization. (‎2018)‎. World Health Organization vaccination coverage cluster surveys: reference manual. World Health Organization. https://apps.who.int/iris/handle/10665/272820. License: CC BY-NC-SA 3.0 IGO
